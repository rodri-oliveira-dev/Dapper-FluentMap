...
- `release-recovery-missing-nuget.yml` recovers future partial NuGet.org publication by running current recovery tooling, checking out the validated commit into a separate source directory, rebuilding artifacts with original release provenance, validating package identities and dependency contracts from the validated source, accepting an existing tag only when it points to that commit, allowing a rollback-deleted missing tag only when NuGet.org proves at least one governed package identity already exists, restoring that missing tag only after successful recovery publication, and publishing only package versions that are genuinely missing.
