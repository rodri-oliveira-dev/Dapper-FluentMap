...
- `release-recovery-missing-nuget.yml` recovers future partial NuGet.org publication by running current recovery tooling, checking out the validated commit into a separate source directory, rebuilding artifacts with original release provenance, accepting an existing tag only when it points to that commit, allowing a rollback-deleted missing tag, restoring that missing tag only after successful recovery publication, and publishing only package versions that are genuinely missing.
