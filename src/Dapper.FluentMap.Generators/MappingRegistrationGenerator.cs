...
namespace Dapper.FluentMap.Generators
{
    ...
    public sealed class MappingRegistrationGenerator : IIncrementalGenerator
    {
        ...
        private static string CreateGeneratedSource(IList<MapCandidate> maps)
        {
            ...
            builder.AppendLine("{");
            AppendGeneratedRegistrationClass(builder, maps, materializerByMap);
            AppendGeneratedMaterializersClass(builder, materializers);
            builder.AppendLine("}");

            return builder.ToString();
        }

        private static void AppendGeneratedRegistrationClass(
            StringBuilder builder,
            IList<MapCandidate> maps,
            IDictionary<string, GeneratedMaterializerInfo> materializerByMap)
        {
        ...
        private static void AppendGeneratedMaterializersClass(
            StringBuilder builder,
            IList<GeneratedMaterializerInfo> materializers)
        {
        ...
        private static void AppendCreateParameterlessNode(
            ...
            bool localAlreadyDeclared)
        {
            if (node.IsRoot || parentLocalName == null || !node.ParentHasPublicGetter)
            {
            ...
            string entityTypeName)
        {
            ...
            if (child.ParentHasPublicSetter && (child.Constructor != null || !child.ParentHasPublicGetter))
            {
                builder.Append(indent);
                builder.Append("    ");
                builder.Append(parentLocalName);
            ...
            builder.Append(indent);
            builder.AppendLine("}");
            if (child.ParentHasPublicSetter && child.ParentCanAssignNull)
            {
                builder.Append(indent);
            ...
            out GeneratedDiagnostic diagnostic)
        {
            ...
            var chainState = new GeneratedMapInvocationChainState(memberPath.TerminalName);
                   ...
                   memberAccess.Parent is InvocationExpressionSyntax chainedInvocation)
            {
                ...
                if (!TryAnalyzeGeneratedMapChainInvocation(
                    chainedInvocation,
                    chainedMethod,
                    mapType,
                    semanticModel,
            ...
            result = new GeneratedMapInvocation(
                memberPath,
                chainState.ColumnName,
                chainState.Ignored,
                chainState.ReadConverter);
            return true;
        }

        private static bool TryAnalyzeGeneratedMapChainInvocation(
            InvocationExpressionSyntax chainedInvocation,
            IMethodSymbol chainedMethod,
            INamedTypeSymbol mapType,
            SemanticModel semanticModel,
            System.Threading.CancellationToken cancellationToken,
            GeneratedMemberPath memberPath,
            GeneratedMapInvocationChainState chainState,
            out string skipReason,
            out GeneratedDiagnostic diagnostic)
        {
            skipReason = null;
            ...
            {
                return chainState.TryApplyColumn(chainedInvocation, semanticModel, cancellationToken, out skipReason);
            }

            if (IsIgnoreInvocation(chainedMethod))
            {
                chainState.ApplyIgnore();
                return true;
            }

            if (IsGeneratedReadConverterInvocation(chainedMethod))
            {
                return TryApplyGeneratedReadConverter(
                    chainedInvocation,
                    chainedMethod,
                    mapType,
                    memberPath,
            ...
            if (IsWriteOnlyConverterInvocation(chainedMethod) ||
                IsReadNeutralPersistenceInvocation(chainedMethod))
            {
                return true;
            }
        ...
        private static bool TryApplyGeneratedReadConverter(
            InvocationExpressionSyntax chainedInvocation,
            IMethodSymbol chainedMethod,
            INamedTypeSymbol mapType,
            GeneratedMemberPath memberPath,
            ...
            out GeneratedDiagnostic diagnostic)
        {
            ...
            if (chainState.ReadConverter != null)
            {
            ...
            if (TryCreateReadConverterBinding(
                ...
                out var readConverter,
                out var converterReason))
            {
                chainState.ApplyReadConverter(readConverter);
                skipReason = null;
                return true;
            }

            return CreateReadConverterFailure(
                chainedInvocation,
                chainedMethod,
                mapType,
                converterReason,
        ...
        private static bool CreateReadConverterFailure(
            InvocationExpressionSyntax chainedInvocation,
            IMethodSymbol chainedMethod,
            INamedTypeSymbol mapType,
            string converterReason,
            out string skipReason,
            out GeneratedDiagnostic diagnostic)
        {
            ...
            out string reason)
        {
            ...
            while (expression != null)
            {
                ...
                {
                    if (!TryPushGeneratedMemberPathProperty(memberAccess, semanticModel, cancellationToken, properties, out expression, out reason))
                    {
                        return false;
                    }
                ...
                if (TryCompleteGeneratedMemberPath(expression, semanticModel, cancellationToken, properties, out memberPath))
                {
                    return true;
                }

                reason = expression is IdentifierNameSyntax
                    ? "the expression must resolve to a property path rooted in the entity parameter"
                    : "the expression must resolve to a property path";
                return false;
            }
        ...
        private static bool TryPushGeneratedMemberPathProperty(
            MemberAccessExpressionSyntax memberAccess,
            SemanticModel semanticModel,
            System.Threading.CancellationToken cancellationToken,
            Stack<IPropertySymbol> properties,
            out ExpressionSyntax nextExpression,
            out string reason)
        {
            nextExpression = null;
            reason = null;
            ...
            nextExpression = StripCastsAndParentheses(memberAccess.Expression);
            return true;
        }

        private static bool TryCompleteGeneratedMemberPath(
            ...
            out GeneratedMemberPath memberPath)
        {
            ...
            if (identifier == null || properties.Count == 0)
            {
                return false;
            }
            ...
            out string skipReason)
        {
            ...
            {
                var candidate = TryCreateConstructorBindingCandidate(node, constructor);
                if (candidate != null)
                {
            ...
            if (TrySelectConstructorBinding(candidates, out constructorBinding))
            {
                return true;
            }
        ...
        private static GeneratedConstructorBinding TryCreateConstructorBindingCandidate(
            GeneratedMaterializationNode node,
            IMethodSymbol constructor)
        {
            ...
            foreach (var parameter in constructor.Parameters)
            {
                ...
                parameters.Add(CreateConstructorParameter(node, parameter, parameterBinding, parameters.Count));
                score += parameterBinding.Score;
            }

            return ConstructorSatisfiesRequiredMembers(node, parameters)
                ? new GeneratedConstructorBinding(constructor, parameters, score)
                : null;
        }

        private static GeneratedConstructorParameter CreateConstructorParameter(
            GeneratedMaterializationNode node,
            IParameterSymbol parameter,
            GeneratedParameterMatch parameterBinding,
            int index)
        {
            var localName = node.IsRoot
                ? CreateUniqueLocalName(parameter.Name, index)
                : "arg" + node.Id.ToString(System.Globalization.CultureInfo.InvariantCulture) + "_" + index.ToString(System.Globalization.CultureInfo.InvariantCulture);

            return new GeneratedConstructorParameter(
                localName,
        ...
        private static bool ConstructorSatisfiesRequiredMembers(
            GeneratedMaterializationNode node,
            IList<GeneratedConstructorParameter> parameters)
        {
            return node.Leaves.Where(leaf => !leaf.CanAssign).All(leaf => parameters.Any(parameter => parameter.Leaf == leaf)) &&
                   node.Children.Where(child => !child.CanAssignToParent).All(child => parameters.Any(parameter => parameter.Child == child));
        }

        private static bool TrySelectConstructorBinding(
            IList<GeneratedConstructorBinding> candidates,
            out GeneratedConstructorBinding constructorBinding)
        {
            constructorBinding = null;
        ...
        private sealed class GeneratedMapInvocationChainState
        {
            internal GeneratedMapInvocationChainState(string columnName)
            {
                ColumnName = columnName;
            }
            ...
            internal GeneratedReadConverterBinding ReadConverter { get; private set; }

            internal bool TryApplyColumn(
                InvocationExpressionSyntax invocation,
                SemanticModel semanticModel,
                System.Threading.CancellationToken cancellationToken,
                out string skipReason)
            {
                if (!TryGetColumn(invocation, semanticModel, cancellationToken, out var column))
                {
            ...
            internal void ApplyReadConverter(GeneratedReadConverterBinding readConverter)
            {
                ReadConverter = readConverter;
            }
        }
        ...
        private sealed class GeneratedPropertyBinding
        {
                ...
                GeneratedReadConverterBinding readConverter)
            {
                ...
                TypeName = typeName;
                HasAssignableSetter = hasPublicSetter;
            ...
            internal bool HasAssignableSetter { get; }

            internal bool CanAssign => HasAssignableSetter;
        ...
        private sealed class GeneratedMaterializationNode
        {
                ...
                bool isRoot)
            {
                ...
                MemberPath = memberPath;
                IsRoot = isRoot;
                ParentHasPublicGetter = parentProperty != null && MappingRegistrationGenerator.HasPublicGetter(parentProperty);
                ParentHasPublicSetter = parentProperty != null && MappingRegistrationGenerator.HasPublicSetter(parentProperty);
                ParentCanAssignNull = parentProperty == null || MappingRegistrationGenerator.CanAssignNull(parentProperty.Type);
            }
            ...
            internal bool ParentHasPublicGetter { get; }

            internal bool ParentHasPublicSetter { get; }

            internal bool ParentCanAssignNull { get; }

            internal bool CanAssignToParent => IsRoot || ParentHasPublicSetter;
